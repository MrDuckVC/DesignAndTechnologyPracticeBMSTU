...
Orientation::dirNum Snake::getDirectionFromPositions(sf::Vector2f pos1,
...
Snake::Snake(Field *field, sf::Texture *textures, float speed)
    : field0(field), tileLength0(field0->getTileLength()), speed0(speed),
      textures0(textures) {
  timer0 = sf::Clock();

  // Голова
  Segment *bufferSegm = new Segment({tileLength0 * 2, 0}, tileLength0,
                                    Segment::Type::Head, Orientation::Right);
  ...
  // Тело
  bufferSegm = new Segment({tileLength0, 0}, tileLength0, Segment::Type::Body,
                           Orientation::Right);
  ...
  // Хвост
  bufferSegm =
      new Segment({0, 0}, tileLength0, Segment::Type::Tail, Orientation::Right);
...
void Snake::update(Orientation::dirNum direction, sf::Vector2f foodPos) {
  ...
  if (deltaTime0 >= speed0) {
    ...
    if (direction != Orientation::reverse(head->getDirection()) &&
        direction != Orientation::Stop) {
    ...
    if (head->getDirection() ==
        (*std::next(segmentList0.begin(), 2))->getDirection()) {
      bufferSegm->setRotation(getCorner(head->getDirection()))
          .setType(Segment::Type::Body);

    } else {
      bufferSegm
          ->setRotation(
              getCorner(head->getDirection(),
                        (*std::next(segmentList0.begin(), 2))->getDirection()))
    ...
    if (!isFoodEaten0) {
      ...
      Orientation::dirNum newDirection = getDirectionFromPositions(
          bufferSegm->position(),
          (*std::prev(segmentList0.end(), 2))->position());
